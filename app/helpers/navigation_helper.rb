...
module NavigationHelper
  def nav_link_classes(path, match_prefix: false)
    base_inactive = "block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 " \
    ...
    base_active = "block py-2 px-3 text-white bg-blue-700 rounded " \
                  "md:bg-transparent md:text-blue-700 md:p-0 md:dark:text-blue-500"

    active = if match_prefix
      request.path.start_with?(path.to_s.chomp("/"))
    else
      current_page?(path)
    end

    active ? base_active : base_inactive
  end
